...
import { isBookingStepComplete, type BookingValidationDraft } from "@shared/booking";
...
const completeDraft: BookingValidationDraft = {
  ...
  preferredTime: "10:00",
  description: "We are renovating a family living space and need a calm, practical direction.",
  privacyConsent: true,
};
...
describe("booking wizard validation", () => {
  it("requires contact details before leaving step one", () => {
    ...
    expect(isBookingStepComplete(1, { ...completeDraft, phone: "" })).toBe(false);
  });

  it("requires space and scope fields in their respective steps", () => {
    expect(isBookingStepComplete(2, completeDraft)).toBe(true);
    expect(isBookingStepComplete(2, { ...completeDraft, area: "" })).toBe(false);
    expect(isBookingStepComplete(3, { ...completeDraft, budget: "" })).toBe(false);
  });
  ...
  it("requires a meaningful project description before scheduling is complete", () => {
    ...
    expect(isBookingStepComplete(4, { ...completeDraft, description: "short" })).toBe(false);
  });

  it("requires explicit privacy consent at the final review step", () => {
    expect(isBookingStepComplete(5, completeDraft)).toBe(true);
    expect(isBookingStepComplete(5, { ...completeDraft, privacyConsent: false })).toBe(false);
  });
});
