  ...
  return (
    <div className="detail-page page-transition">
      ...
      <section className="detail-story section-pad" id="story">
        <div className="section-marker"><span>01</span><span>THE STORY</span></div>
        <div className="story-columns"><div><p className="eyebrow">الفكرة</p><h2>حين تصبح الخامة<br /><em>جزءاً من الإحساس.</em></h2></div><div className="story-body"><p>{detail.description}</p><p>كل قرار هنا له وظيفة مزدوجة: أن يخدم اليوم العادي، وأن يظل جميلاً حين يتغير الضوء. لذلك جاءت التفاصيل قليلة، لكن حضورها واضح.</p></div></div>
      </section>
      <section className="detail-info section-pad">
        <div className="info-block"><span className="footer-label">الخدمات</span>{detail.services.map((service) => <span className="info-line" key={service}>{service}</span>)}</div>
        <div className="info-block"><span className="footer-label">المفردات</span>{detail.palette.map((color) => <span className="info-line" key={color}>{color}</span>)}</div>
        <div className="info-block info-mark"><div className="monogram-small"><span /></div><span>GLLORIA<br />INTERIORS / STUDIO</span></div>
      </section>
      ...
      <section className="detail-cta section-pad"><Check size={18} /><p>لديكِ مساحة تريدين أن تحكي عنها؟</p><Link href="/contact" className="dark-button">احكي لنا عنها <ArrowUpLeft size={17} /></Link></section>
    </div>
  );
}
