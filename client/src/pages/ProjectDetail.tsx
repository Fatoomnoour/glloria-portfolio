...
function ProjectGallery({
  ...
  return (
    <section
      className="detail-gallery section-pad"
      aria-labelledby="image-archive-title"
    >
      <div className="section-marker" id="image-archive-title">
...
function MaterialExplorer({
  ...
  return (
    <div className="material-explorer" aria-labelledby="materials-title">
      ...
      <div
        ...
        role="tablist"
        aria-label={
          ar ? "الخامات المذكورة في المشروع" : "Materials named in this project"
        }
      >
      ...
      <p className="material-selected" aria-live="polite">
        {items[activeIndex]}
      </p>
    </div>
  );
...
function CaseStudySections({
  ...
  const blocks = [
    {
      label: ar ? "التحدي التصميمي" : "THE DESIGN CHALLENGE",
      value: project.challenge,
    },
    { label: ar ? "فكرة التصميم" : "THE DESIGN IDEA", value: project.concept },
    {
      label: ar ? "الألوان والخامات" : "PALETTE & MATERIALS",
      value: project.materials || project.palette,
    },
    { label: ar ? "نطاق العمل" : "SCOPE OF WORK", value: project.serviceScope },
  ].filter((block): block is { label: string; value: string } =>
    Boolean(block.value?.trim())
  );

  return (
    <section
      className="case-study-sections section-pad"
      aria-labelledby="case-study-title"
    >
      <div className="case-study-intro">
        <div className="section-marker" id="case-study-title">
          <span>02</span>
          <span>{ar ? "دراسة المشروع" : "CASE STUDY"}</span>
        </div>
      ...
      <div className="case-study-content">
        ...
        {project.materials?.trim() && (
          <MaterialExplorer materials={project.materials} ar={ar} />
        )}
        {hasBeforeAfterPair(project) && (
          ...
          >
            <div className="before-after-heading">
              <p className="eyebrow" id="before-after-title">
            ...
            <div className="before-after-grid">
              ...
              <figure>
                <ResponsiveImage
                  ...
                  loading="lazy"
                />
                <figcaption>{ar ? "بعد" : "After"}</figcaption>
              </figure>
            </div>
          </div>
        )}
...
export default function ProjectDetail() {
  ...
  const ar = locale === "ar";
  const { data: managedProject, isLoading } = trpc.projects.bySlug.useQuery(
    { slug },
    { enabled: Boolean(slug) }
  );

  const renderState = getPublicProjectRenderState(
    isLoading,
  ...
  const coverAlt =
    project.imageAlt ||
    `${project.title} — ${ar ? "صورة المشروع" : "project image"}`;
  const gallery = parseProjectGallery(
    project.galleryJson,
    project.imageUrl,
    coverAlt
  );
  const imageKind =
    project.imageKind || (ar ? "صورة المشروع" : "Project image");
  ...
  return (
    <div className="detail-page page-transition">
      <div className="detail-back section-pad">
        <Link href="/projects">
          <ChevronLeft size={16} /> {ar ? "العودة إلى الأعمال" : "Back to work"}
        </Link>
        <span>GLL / {projectNumber}</span>
      </div>
      ...
      <ProjectGallery
        ...
        imageKind={imageKind}
        ar={ar}
        markerNumber={showCaseStudy ? "03" : "02"}
      />
      ...
      <section className="detail-cta section-pad">
        <Check size={18} />
        <p>
          {ar ? "أعجبك إحساس هذه المساحة؟" : "Does this space feel like you?"}
        </p>
        <Link
          href={`/booking?project=${encodeURIComponent(project.slug)}`}
          className="dark-button"
        >
          {ar ? "أريد مساحة بهذا الإحساس" : "I want a space with this feeling"}{" "}
          <ArrowUpLeft size={17} />
        </Link>
      </section>
    </div>
