...
export default function TestimonialsSection() {
  ...
  return (
    <section
      className={`testimonials section-pad ${hasTestimonials ? "has-testimonials" : "is-quiet"}`}
      id="testimonials"
    >
      ...
      <div className="testimonials-layout">
        <div className="testimonial-heading">
          ...
          <h2>
            ...
            <br />
            <em>{t("testimonials.accent")}</em>
          </h2>
          <p>{t("testimonials.body")}</p>
          <Link className="text-link" href="/booking">
            {t("testimonials.cta")} <ArrowUpLeft size={16} />
          </Link>
        </div>
        <div
          className={
            hasTestimonials ? "testimonial-carousel" : "testimonial-empty"
          }
        >
          {isLoading ? (
            ...
            <div className="testimonial-slide" aria-live="polite">
              <article className="testimonial-card">
                ...
                <div className="testimonial-author">
                  <strong>{active.clientName}</strong>
                  <span>
                    {[active.clientRole, active.projectName]
                      .filter(Boolean)
                      .join(" · ") ||
                      (locale === "ar" ? "عميل Glloria" : "Glloria client")}
                  </span>
                </div>
                <span className="verified-note">VERIFIED CLIENT STORY</span>
              </article>
              <div className="testimonial-controls">
                ...
                <button
                  ...
                  onClick={() => move(1)}
                  aria-label={
                    locale === "ar" ? "التجربة التالية" : "Next client story"
                  }
                >
                  <ChevronLeft size={17} />
                </button>
            ...
            <div className="testimonial-quiet">
              <div className="quote-mark">
                <Quote size={20} />
              </div>
              <p>{t("testimonials.empty")}</p>
              <span className="verified-note">
                REAL CLIENT STORIES / PRIVATE BY DEFAULT
              </span>
            </div>
          )}
        </div>
