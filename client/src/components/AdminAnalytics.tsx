...
function getBookingConfig(locale: "ar" | "en") {
  return {
    bookings: {
      ...
      theme: { light: "#b65e45", dark: "#d38b70" },
    },
    confirmed: {
      label: locale === "ar" ? "المؤكدة" : "Confirmed",
      theme: { light: "#7b8064", dark: "#b6bd95" },
    },
  };
}
...
function getProjectConfig(locale: "ar" | "en") {
  return {
    interiorProjects: {
      label: locale === "ar" ? "تصميم داخلي" : "Interior design",
      theme: { light: "#b65e45", dark: "#d38b70" },
    },
    architecturalProjects: {
      label: locale === "ar" ? "تصميم معماري" : "Architecture",
      theme: { light: "#7b8064", dark: "#b6bd95" },
    },
  };
}
  ...
  const totals = useMemo(
    () =>
      points.reduce(
        ...
        { bookings: 0, projects: 0 }
      ),
    [points]
  );

  const rangeLabel = (value: AnalyticsRange) =>
    locale === "ar" ? `${value} شهراً` : `${value} months`;
    ...
    <section className="admin-analytics" aria-labelledby="analytics-title">
      <div className="admin-analytics-head">
        <div>
          ...
          <h2 id="analytics-title">
            {locale === "ar" ? "نبض الاستوديو" : "Studio pulse"}
          </h2>
          <p>
            {locale === "ar"
              ? "قراءة هادئة لنمو الحجوزات والأعمال، مبنية على البيانات المحفوظة فعلياً."
              : "A quiet view of booking activity and archive growth, based on saved records."}
          </p>
        </div>
        <div
          className="analytics-range"
          role="group"
          aria-label={locale === "ar" ? "الفترة الزمنية" : "Time range"}
        >
      ...
      <div className="analytics-kpis">
        <div className="analytics-kpi">
          <span>
            ...
            {locale === "ar" ? "إجمالي الحجوزات" : "Bookings in range"}
          </span>
          <strong>{formatValue(totals.bookings)}</strong>
          <small>
            <TrendingUp size={13} />{" "}
            {locale === "ar" ? "حسب الفترة المختارة" : "Selected time range"}
          </small>
        </div>
        <div className="analytics-kpi">
          <span>
            <FolderKanban size={15} />{" "}
            {locale === "ar" ? "المشاريع المضافة" : "Projects added"}
          </span>
          <strong>{formatValue(totals.projects)}</strong>
          <small>
            <BarChart3 size={13} />{" "}
            {locale === "ar"
              ? "يشمل المنشور والمسودة"
              : "Published and draft records"}
          </small>
        </div>
      </div>
      ...
      {analyticsQuery.isLoading ? (
        ...
        <div className="analytics-state analytics-error">
          {locale === "ar"
            ? "تعذر تحميل التحليلات حالياً. حاول تحديث الصفحة."
        ...
        <div className="analytics-chart-grid">
          <article className="analytics-card analytics-card-wide">
            <div className="analytics-card-head">
              <div>
                <span className="admin-kicker">INQUIRIES / TREND</span>
                <h3>
                  {locale === "ar"
                    ? "الحجوزات بمرور الوقت"
                    : "Bookings over time"}
                </h3>
              </div>
            ...
            <ChartContainer className="analytics-chart" config={bookingConfig}>
              ...
              >
                ...
                <Line
                  ...
                  strokeWidth={2.5}
                  dot={{ r: 3, strokeWidth: 0, fill: "var(--color-bookings)" }}
                  activeDot={{ r: 5 }}
                />
                <Line
                  type="monotone"
                  ...
                  strokeWidth={2}
                  dot={{ r: 3, strokeWidth: 0, fill: "var(--color-confirmed)" }}
                  activeDot={{ r: 5 }}
                />
              </LineChart>
            </ChartContainer>
          ...
          <article className="analytics-card analytics-card-wide">
            <div className="analytics-card-head">
              <div>
                <span className="admin-kicker">ARCHIVE / GROWTH</span>
                <h3>
                  {locale === "ar"
              ...
              <span className="analytics-card-note">
                {locale === "ar" ? "حسب النوع" : "By discipline"}
              </span>
            </div>
            <ChartContainer className="analytics-chart" config={projectConfig}>
              ...
              >
                ...
                <Legend verticalAlign="top" height={30} />
                <Bar
                  dataKey="interiorProjects"
                  stackId="projects"
                  fill="var(--color-interiorProjects)"
                  radius={[0, 0, 0, 0]}
                />
                <Bar
                  dataKey="architecturalProjects"
                  stackId="projects"
                  fill="var(--color-architecturalProjects)"
                  radius={[3, 3, 0, 0]}
                />
              </BarChart>
            </ChartContainer>
