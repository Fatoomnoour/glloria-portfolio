...
# Post-audit production fixes — booking, WhatsApp, Vercel, and images

- [x] Reproduce and fix the reported blank `/booking` route without weakening the existing five-step persisted booking contract, privacy safeguards, validation, or attribution. (A missing project lookup now resolves to `null` rather than `undefined`, preventing React Query from throwing; direct, valid-project, and invalid-slug mobile checks render the booking page.)
- [x] Improve Arabic/English prefilled WhatsApp messages for public contact and successful booking follow-up; preserve browser-initiated handoff only, never automatic server sending. (General CTA links now start with a localized ready-made message; the success handoff retains the request reference and booking details.)
- [ ] Inspect and activate an authorised Vercel connection if available, deploy the private repository, configure required environment-variable names securely, register the exact OAuth callback after the host is known, and test login/logout on the real `*.vercel.app` URL.
- [x] Inventory original image availability and dimensions; create and upload responsive WebP/AVIF derivatives only when the approved source files can be retrieved safely, otherwise document and implement a truthful fallback path. (19 approved originals were retrieved via the current storage proxy; 65 proportion-preserving WebP variants were created and uploaded to permanent WebDev storage, with precise `srcSet` and intrinsic dimensions plus original-image fallback.)
- [ ] Add focused tests, run functional/visual/Lighthouse QA, update deployment and QA documentation, push verified source to private GitHub, and save a checkpoint.
